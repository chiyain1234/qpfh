...
module.exports = {
    ...
    data: new SlashCommandSubcommandBuilder()
        .setName('shrinkgif')
        .setDescription('指定したユーザーのアバターを縮ませるGIFを生成します。')
    ...
    async execute(interaction, client) {
        try {
            ...
            for (let frame = 0; frame < frames; frame++) {
                ...
                const scale = Math.sin(frame * Math.PI / frames); // サイン関数を使用して縦方向のスケールを変化させる
                const width = canvas.width;
                const height = avatarBuffer.height * (1 - scale); // 逆に縮むようにするため、スケールを1から引く
                const x = 0;
            ...
            // メッセージにGIFを添付して送信
            const attachment = new MessageAttachment(buffer, 'shrinkup.gif');
            const embed = new MessageEmbed()
                .setTitle(`${user.username} is shrinking up.`)
                .setImage('attachment://shrinkup.gif')
